...
CREATE TABLE IF NOT EXISTS pgedge_vectorizer.queue (
    id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_queue_chunk ON pgedge_vectorizer.queue(chunk_table, chunk_id);

CREATE INDEX IF NOT EXISTS idx_queue_created_at ON pgedge_vectorizer.queue(created_at)
...
CREATE OR REPLACE FUNCTION pgedge_vectorizer.generate_embedding(
    query_text TEXT
) RETURNS vector
...
CREATE OR REPLACE FUNCTION pgedge_vectorizer.detect_embedding_dimension()
RETURNS INT
...
CREATE OR REPLACE VIEW pgedge_vectorizer.queue_status AS
SELECT
    chunk_table,
    status,
    COUNT(*) as count,
...
-- Failed items view
CREATE OR REPLACE VIEW pgedge_vectorizer.failed_items AS
SELECT
    id,
    chunk_table,
    chunk_id,
...
-- Pending items count
CREATE OR REPLACE VIEW pgedge_vectorizer.pending_count AS
SELECT
...
-- Retry failed items
CREATE OR REPLACE FUNCTION pgedge_vectorizer.retry_failed(
...
CREATE OR REPLACE FUNCTION pgedge_vectorizer.clear_completed(
...
CREATE OR REPLACE FUNCTION pgedge_vectorizer.reprocess_chunks(
